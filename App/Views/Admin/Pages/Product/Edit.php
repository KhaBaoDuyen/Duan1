                           ...
                           <form class="form-horizontal" action="/admin/products/<?= $data['product']['product_id'] ?>" method="POST" enctype="multipart/form-data">
                                <div class="card-body">
                                    ...
                                    <div align="center" class="form-group">
                                        ...
                                        <img src="<?= APP_URL ?>/public/uploads/products/<?= $data['product']['image'] ?>" alt="" width="200px">
                                    </div>
                                    <input type="hidden" name="method" id="" value="PUT">
                                    <div class="form-group">
                                        <label for="id">ID</label>
                                        <input type="text" class="form-control" id="id" value="<?= $data['product']['product_id'] ?>" name="id" disabled>
                                    </div>
                                    <div class="form-group ">
                                        <label for="name">Tên*</label>
                                        <input type="text" class="form-control" id="name" value="<?= $data['product']['name'] ?>" placeholder="Nhập tên sản phẩm..." name="name" >
                                    </div>
                                    <div class="form-group ">
                                    ...
                                    <div class="form-group ">
                                        ...
                                        <input type="number" class="form-control" id="price" value="<?= $data['product']['price'] ?>" placeholder="Nhập giá tiền..." name="price"  min="0">
                                    </div>
                                    <div class="form-group ">
                                        <label for="discount_price">Giá giảm*</label>
                                        <input type="number" class="form-control" id="discount_price" value="<?= $data['product']['discount_price'] ?>" placeholder="Nhập giá giảm..." name="discount_price"  min="0">
                                    </div>
                                    <div class="form-group ">
                                    ...
                                    <div class="form-group ">
                                        ...
                                        <select class="select2 form-select shadow-none" style="width: 100%; height:36px;" id="category_id" name="categories_id" >
                                            <option value="" selected disabled>Vui lòng chọn...</option>
                                            <?php
                                            foreach ($data['category'] as $item) :
                                            ?>
                                                <option value="<?= $item['categories_id'] ?>" <?= ($item['categories_id'] == $data['product']['category_id']) ? 'selected' : '' ?>><?= $item['name'] ?></option>
